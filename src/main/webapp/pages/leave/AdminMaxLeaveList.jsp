...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		// 实例化数据网格
		$("#dg").datagrid({
			url : "${pageContext.request.contextPath }/leave/getUserLeaveListAll",
			pagination : true,
			rownumbers : true,
			pageSize : 5,
			pageList : [ 5, 10, 15, 20 ],
			...
			toolbar : [ {
				...
				handler : function() {
					...
					if(parseInt(selectedRows[0].leaveStatus) >= 5){
						$.messager.show({
							msg : '已处理的请假信息，无法同意批假',
							title : '消息提示'
						});
				...
				handler : function() {
					...
					if(parseInt(selectedRows[0].leaveStatus) > 4){
						$.messager.show({
							msg : '已处理的请假信息，无法驳回',
							title : '消息提示'
						});
				...
				id : "btn_exportExcal",
				text : "导出所有通过的假条到excal",
				iconCls : "icon-edit",
				handler : function() {
					
					//alert("导出信息到excal");
					window.location.href = "${pageContext.request.contextPath}/leave/downloadLeaveExcel?leaveId="+${sessionScope.sysUser.did};
			...
			} ,
			{
				...
				handler : function() {
					...
					window.location.href = "${pageContext.request.contextPath}/leave/downloadLeaveWord?leaveId="+${sessionScope.sysUser.did};
				}
			}
			]
